...
R.ok('each split row carries a coach window from subAttendanceWindow', /rows\.push\(\{ m, sport: sp, coachId: parseInt\(cid\), window: \{ from: w\.openStart \? null : w\.from, to: _to \}, attKey: attKeyForSport\(m, sp, cid\)(?:, hist: _hist, switchedAway: _switchedAway, histTo: _histTo)? \}\)/.test(src));
