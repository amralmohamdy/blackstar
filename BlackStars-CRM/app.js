...
function computeMemberCommissions(ym) {
  ...
  for (const inv of invs) {
    ...
    for (const li of lineItems) {
      ...
      // v6.570 — MIXED: fan the single Mixed line out into ONE row PER coach who taught, each crediting
      // (Mixed price / total classes) × the classes they taught. So the report shows the real split.
      if (li.sport === MIXED) {
        ...
        for (const _cid of mixedCoachesTaught(mem, inv, li, null)) {
          const _taught = mixedCappedByCoach(mem, inv, li, _cid, null, null);   // v6.571: capped at paid classes
          if (_taught <= 0) continue;
