...
function mixedCoachFeeShare(mem, inv, li, coachId, uptoDate) {
  ...
  const taught = mixedAttendedByCoach(mem, coachId, w.from, to);
    ...
    for (const li of lineItems) {
      ...
      if (li.sport === MIXED) {
        ...
        for (const _cid of mixedCoachesTaught(mem, inv, li, null)) {
          const _w = mixedWindow(mem, inv, li);
          const _taught = mixedAttendedByCoach(mem, _cid, _w.from, _w.to);
    ...
    for (const li of lineItems) {
      ...
      if (li.sport === MIXED) {
        ...
        const _taught = uptoDate
          ? mixedAttendedByCoach(mem, coachId, _w.from, ((!_w.to || uptoDate < _w.to) ? uptoDate : _w.to))
          : mixedAttendedByCoachInMonth(mem, coachId, monthKey, _w.from, _w.to);
        if (_taught > 0) {
      ...
      if (uptoDate) {
        ...
        const attMonth = attendedYInMonth(mem, li.sport, monthKey, _cw.from, _cw.to, undefined, sub.coachId);   // v6.534: coach-aware cell
        if (attMonth > 0) {
