...
// 1 month, 5 weeks, 2 months, 3 months, 6 months (day-counts 1/7/14/30/35/60/90/180). The STORED
// value stays the day count (expiry math unchanged); a non-preset legacy value is preserved as its
// own option. (v6.553: added "5 weeks" = 35 days.)
const H = require('./qc-harness.js');
const R = H.reporter('v6.521 · membership validity presets');
const src = H.readSrc();

R.section('source — options + helpers');
R.ok('VALIDITY_OPTIONS is the 8 requested day-counts (incl. 5 weeks=35)', /const VALIDITY_OPTIONS = \[1, 7, 14, 30, 35, 60, 90, 180\];/.test(src));
...
R.section('runtime — labels + option list');
{
  ...
  const pairs = [[1, '1 day'], [7, '1 week'], [14, '2 weeks'], [30, '1 month'], [35, '5 weeks'], [60, '2 months'], [90, '3 months'], [180, '6 months']];
  ...
  R.ok('option list has all 8 presets', (html30.match(/<option /g) || []).length === 8, 'count=' + (html30.match(/<option /g) || []).length);
  R.ok('30 (1 month) is the selected default', /<option value="30" selected>1 month<\/option>/.test(html30));
  R.ok('5 weeks (35 days) is an option', /<option value="35"[^>]*>5 weeks<\/option>/.test(html30));
  ...
  R.ok('preserving a legacy value yields 9 options (8 presets + the legacy)', (html45.match(/<option /g) || []).length === 9, 'count=' + (html45.match(/<option /g) || []).length);
}
