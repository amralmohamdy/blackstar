...
R.ok('the KPI (clubAttended) also uses attKey + inWin (same basis)', /const dd = \(attKey === MIXED\) \? mixedDayMarks\(m, mk\) : \(m\.dailyAttendance\?\.\[mk\]\?\.\[_k\] \|\| \{\}\);/.test(src) && /const _k = attKey \|\| sport;/.test(src) && /if \(!inWin\(window, mk, k\)\) continue;/.test(src));
