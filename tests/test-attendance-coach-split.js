...
R.ok('each split row carries a coach window from subAttendanceWindow', /rows\.push\(\{ m, sport: sp, coachId: parseInt\(cid\), window: \{ from: w\.openStart \? null : w\.from, to: _to \}, attKey: attKeyForSport\(m, sp, cid\) \}\)/.test(src));
