...
function findSubForLine(m, inv, li) {
  ...
  // 1. Exact invoice-ref link — the strongest signal (a sub that names this invoice).
  const byRef = subs.find(s => s.invoiceNumber === inv.ref && s.activity === li.sport);
  ...
  for (const inv of state.invoices) {
    ...
    for (const li of lineItems) {
      ...
      if (uptoDate) {
        ...
        if (ended && remaining > 0 && attended > 0 && !settledMonth) {   // expired WITH ≥1 attended → pay the rest in full
        ...
        if (endMonth === monthKey && ended && remaining > 0 && attendedAll > 0 && !settledMonth) {   // v6.445: only if ≥1 attended
          base += perClass * remaining;
          lines.push({ memberName, mid, sport: li.sport, kind: 'trueup', classes: remaining, perClass,
...
const ROLE_ALLOWED = {
  ...
  receptionist: [
    'members', 'families', 'expiring', 'completed', 'duepayment', 'trials', 'reminders',
