...
const SCHEMA_VERSION = 10;      // v10: add the "Mixed" special sport to the sports list
...
const DEFAULT_SPORTS = ['MMA','Boxing','Kick Boxing','Karate','Taekwondo','Gymnastic','Football','Swimming','Zumba','Summer Camp','Mixed'];
...
// v6.570 — "Mixed" is a special sport: ONE package to TRY MANY sports, each class possibly under a
// DIFFERENT coach. It behaves like a normal sport for enrollment/invoice/due (one price + one class
// count), but its attendance records the ACTUAL sport + coach per day (m.mixedAttendance) and its
// commission SPLITS across the real coaches by classes each taught — the opposite of Summer Camp,
// which earns no commission. Every Mixed-specific branch is gated on this constant so existing sports
// are unaffected.
const MIXED = 'Mixed';
...
const DEFAULT_EXPENSE_CATEGORIES = [
  'Bank Commission','Citadel Company Share','Equipment','Products','Cleaning','Utilities','Marketing','Subscriptions',
  'Transport','Operations','Rent','Maintenance','Salary','Refund',
  'Cash collected by owner','Owner Net Profit','Others',
];
...
// 'Citadel Company Share' backs the auto-posted monthly facility-company payment. (v6.577)
const RESERVED_EXPENSE_CATEGORIES = ['Refund', 'Cash collected by owner', 'Citadel Company Share', 'Others'];
...
Object.defineProperty(globalThis, 'EXP_CATS', {
  get() {
    ...
    // v6.582 — pin the auto-managed "Citadel Company Share" directly AFTER "Bank Commission" so both
    // auto bank expenses sit together at the top (old installs had it inserted near the bottom, before
    // "Others"). Purely cosmetic ordering — no data change.
    const CIT = 'Citadel Company Share', bcIdx = out.indexOf('Bank Commission'), citIdx = out.indexOf(CIT);
    if (bcIdx >= 0 && citIdx >= 0 && citIdx !== bcIdx + 1) {
      out.splice(citIdx, 1);
      out.splice(out.indexOf('Bank Commission') + 1, 0, CIT);
    }
    return out;
  },
});
...
const VALIDITY_OPTIONS = [1, 7, 14, 30, 35, 60, 90, 180];
...
const VALIDITY_PRESET_LABELS = {
  ...
  30: ['1 month', 'شهر'], 35: ['5 weeks', '5 أسابيع'], 60: ['2 months', 'شهران'], 90: ['3 months', '3 أشهر'], 180: ['6 months', '6 أشهر'],
};
...
// ── OVERPAYMENT integrity (v6.538) ───────────────────────────────────────────────
// An invoice whose payment rows sum to MORE than its charge (invoiceTotal) — the "Paid > Charged"
// class. Typically a SECOND collection that re-paid an amount already covered (a stale multi-device
// due, or a switch remainder paid twice). Unlike a phantom (same amount+date+method as a real row),
// the extra here looks like a genuine row on a different date, so it is NOT auto-removed. The safe
// correction is APPEND-ONLY: a negative _correction row that brings amountPaid down to the invoice
// total — no row is ever deleted, the full history is kept, and it is reversible. The admin reviews
// and clicks per invoice; if the excess is a REAL prepayment the admin simply leaves it as a credit.
// The member's NET overpayment across ALL their membership invoices — POSITIVE = paid more than charged.
// Netted exactly like memberOutstanding (same legacy-skip, same invoiceTotal), so an overpayment on one
// invoice that merely covers another is NOT falsely flagged, and a credit-note (negative) invoice is
// accounted on both sides. This is the authoritative "Paid > Charged" measure.
function _memberOverpayExcess(memberId) {
  const invs = (state.invoices || []).filter(i => !i.deleted && i.customerId === memberId && (i.category || 'Membership') === 'Membership');
  let net = 0, any = false;
  for (const i of invs) {
    if (i.amountPaid == null && !(Array.isArray(i.payments) && i.payments.length)) continue;   // legacy fully-paid → 0 both sides
    any = true;
    net += (invoiceTotal(i) - invoicePaid(i));
  }
  return any ? Math.round(-net * 100) / 100 : 0;   // -net: positive when Paid exceeds Charged
}
// One entry per MEMBER whose net Paid exceeds net Charged. `inv` is the single most-overpaid invoice — where
// the append-only correction lands. `total`/`sum` are the member's net charge / net paid; `excess` the overage.
function _paymentOverpayIssues() {
  const out = [], seen = new Set();
  for (const inv of (state.invoices || [])) {
    if (!inv || inv.deleted) continue;
    const mid = inv.customerId;
    if (mid == null || (inv.category || 'Membership') !== 'Membership') continue;   // walk-in/product handled elsewhere
    if (seen.has(mid)) continue; seen.add(mid);
    const excess = _memberOverpayExcess(mid);
    if (excess <= 0.5) continue;
    const invs = (state.invoices || []).filter(i => !i.deleted && i.customerId === mid && (i.category || 'Membership') === 'Membership');
    let target = null, best = 0;
    for (const i of invs) { const over = invoicePaid(i) - invoiceTotal(i); if (over > best) { best = over; target = i; } }
    if (!target) continue;
    const sum = Math.round(invs.reduce((s, i) => s + invoicePaid(i), 0) * 100) / 100;
    const m = (state.members || []).find(x => x.id === mid);
    out.push({ inv: target, memberId: mid, member: (m && m.name) || target.customerName || ('#' + mid), total: Math.round((sum - excess) * 100) / 100, sum, excess });
  }
  return out;
}
// Append a negative correction row = the MEMBER's net overpayment, onto its most-overpaid invoice, so the
// member's net Paid drops to net Charged. NEVER deletes a row; append-only, reversible. Returns excess corrected.
function _correctInvoiceOverpay(inv) {
  if (!inv || !Array.isArray(inv.payments)) return 0;
  const excess = (inv.customerId != null)
    ? _memberOverpayExcess(inv.customerId)
    : Math.round((inv.payments.reduce((s, p) => s + (Number(p.amount) || 0), 0) - (typeof invoiceTotal === 'function' ? invoiceTotal(inv) : (Number(inv.amount) || 0))) * 100) / 100;
  if (excess <= 0.5) return 0;
  const today = (typeof TODAY !== 'undefined' && TODAY) ? TODAY : new Date().toISOString().slice(0, 10);
  inv.payments.push({
    amount: -excess, date: today, month: String(today).slice(0, 7), method: 'cash', _correction: true,
    note: 'Overpayment correction — payments exceeded the invoice total',
    by: (typeof currentUserId === 'function' ? currentUserId() : undefined),
    byName: (typeof currentUserName === 'function' ? currentUserName() : undefined),
...
// ── SWITCH / COACH-CHANGE REVIEW + PAID-BUT-UNINVOICED REPAIR (v6.541) ────────────
// Coaches leaving the club (Iyad → Abdel Salam, Mostafa → Zakaria) produced two shapes of data:
//   • TAGGED switches (done via the Switch tool — carry switchedAwayTo/switchedFrom markers), and
//   • MANUAL coach changes (a new coach sub added, the old one left/completed — NO markers), which the
//     switch-repair tools couldn't "see". Both are surfaced here for REVIEW (read-only; no mutation).
function _coachChangeReview() {
  const out = [];
  for (const m of (state.members || [])) {
    if (m.deleted) continue;
    const bySport = {};
    for (const s of (m.subscriptions || [])) { if (!s.activity) continue; (bySport[s.activity] = bySport[s.activity] || []).push(s); }
    for (const sp in bySport) {
      const subs = bySport[sp].slice().sort((a, b) => String(a.start || '').localeCompare(String(b.start || '')));
      const cs = [...new Set(subs.map(s => String(s.coachId)).filter(x => x && x !== 'null' && x !== 'undefined'))];
      if (cs.length < 2) continue;
      const tagged = subs.some(s => s.switchedAwayTo || s.switchedFrom);
      out.push({ memberId: m.id, member: m.name, sport: sp, coachIds: cs, coaches: cs.map(id => (typeof coachName === 'function' ? coachName(id) : id)), tagged });
    }
  }
  return out;
}
// A sport that is ENROLLED (profile) and genuinely PAID but has NO invoice/line — the payment landed on
// another invoice as an untagged overpayment (Ali Salem's MMA 500 sat on his old KB invoice INV639132).
// Precise: only flagged when a matching overpayment exists to source it from, so it is a real money bug.
function _paidUninvoicedSports() {
  const out = [], round = x => Math.round((Number(x) || 0) * 100) / 100;
  for (const m of (state.members || [])) {
    if (m.deleted) continue;
    const invs = (state.invoices || []).filter(i => !i.deleted && String(i.customerId) === String(m.id) && (i.category || 'Membership') === 'Membership');
    const covered = new Set();
    for (const i of invs) { for (const l of (i.lineItems || [])) if (l.sport) covered.add(l.sport); if (i.sport) covered.add(i.sport); }
    for (const e of (m.enrollments || [])) {
      if (!e.sport || covered.has(e.sport) || !(Number(e.price) > 0)) continue;
      let src = null, bestExcess = 0;
      for (const i of invs) { const ex = round((typeof invoicePaid === 'function' ? invoicePaid(i) : 0) - (typeof invoiceTotal === 'function' ? invoiceTotal(i) : 0)); if (ex > bestExcess) { bestExcess = ex; src = i; } }
      if (src && bestExcess >= round(e.price) - 1) out.push({ memberId: m.id, member: m.name, sport: e.sport, coachId: e.coachId, price: round(e.price), srcInvId: src.id, srcRef: src.ref, excess: bestExcess });
    }
  }
  return out;
}
// Repair ONE paid-but-uninvoiced sport: create its Membership invoice + subscription from the enrollment,
// and RELOCATE the misplaced money — append a −price correction on the source invoice (drops its overpayment)
// and record the payment on the new invoice (tagged to the sport). Append-only, money-conserving, reversible.
function _repairPaidUninvoiced(memberId, sport) {
  const m = (state.members || []).find(x => String(x.id) === String(memberId)); if (!m) return null;
  const e = (m.enrollments || []).find(x => x.sport === sport); if (!e) return null;
  const c = _paidUninvoicedSports().find(x => String(x.memberId) === String(memberId) && x.sport === sport); if (!c) return null;
  const src = (state.invoices || []).find(i => i.id === c.srcInvId); if (!src) return null;
  const price = c.price;
  const today = (typeof TODAY !== 'undefined' && TODAY) ? TODAY : new Date().toISOString().slice(0, 10);
  const start = e.start || today;
  const ref = (typeof nextInvoiceRef === 'function') ? nextInvoiceRef() : ('INV' + Date.now());
  const cName = (typeof coachName === 'function') ? coachName(e.coachId) : '';
  const inv = {
    id: (typeof nextId === 'function' ? nextId(state.invoices) : Date.now()), ref, date: start, month: String(start).slice(0, 7),
    description: `${m.name} — ${sport} subscription`, amount: price, amountPaid: price,
    category: 'Membership', activityType: 'subscription', customerId: m.id, customerName: m.name, coachId: e.coachId, coach: cName,
    lineItems: [{ sport, coachId: e.coachId, coach: cName, classes: e.classes, price }],
    payments: [{ amount: price, date: today, month: String(today).slice(0, 7), method: 'cash', sport, note: 'Relocated from ' + src.ref + ' (misplaced overpayment)', by: (typeof currentUserId === 'function' ? currentUserId() : undefined), byName: (typeof currentUserName === 'function' ? currentUserName() : undefined), at: new Date().toISOString() }],
  };
  if (typeof stampUpdate === 'function') stampUpdate(inv);
  state.invoices.push(inv);
  const subEnd = (typeof addDays === 'function' && start && e.validity) ? addDays(start, parseInt(e.validity) || 30) : (e.end || null);
  (m.subscriptions = m.subscriptions || []).push({ activity: sport, coachId: e.coachId, coach: cName, start, end: subEnd, validity: e.validity, status: 'active', totalClasses: e.classes || null, attendedClasses: 0, amountPaid: price, invoiceNumber: ref, _sid: 's' + Date.now() });
  if (typeof stampUpdate === 'function') stampUpdate(m);
  (src.payments = src.payments || []).push({ amount: -price, date: today, month: String(today).slice(0, 7), method: 'cash', _correction: true, note: `Moved ${sport} payment to ${ref}`, by: (typeof currentUserId === 'function' ? currentUserId() : undefined), byName: (typeof currentUserName === 'function' ? currentUserName() : undefined), at: new Date().toISOString() });
  src.amountPaid = Math.round(src.payments.reduce((s, p) => s + (Number(p.amount) || 0), 0) * 100) / 100;
  if (typeof stampUpdate === 'function') stampUpdate(src);
  return { ref, moved: price, from: src.ref };
}
if (typeof window !== 'undefined') {
  ...
  window._paymentOverpayIssues = _paymentOverpayIssues;
  window._correctInvoiceOverpay = _correctInvoiceOverpay;
  window._memberOverpayExcess = _memberOverpayExcess;
  window._coachChangeReview = _coachChangeReview;
  window._paidUninvoicedSports = _paidUninvoicedSports;
  window._repairPaidUninvoiced = _repairPaidUninvoiced;
}
...
function save() {
  ...
  try {
    ...
    // Capacity check on the serialized payload (the dominant localStorage user). v6.593 — measure what
    // ACTUALLY goes to localStorage: the audit log is no longer cached locally when the cloud is active
    // (it lives in Firestore), so exclude it or the warning over-reports (it was the ~3.4MB that pushed
    // the bar to 95% even though localStorage no longer holds it).
    let approxBytes = 0;
    try { const { auditLog, ...localish } = stateToSave; approxBytes = JSON.stringify(localish).length; } catch (_) { try { approxBytes = JSON.stringify(stateToSave).length; } catch (_2) {} }
...
async function withCloudConfirm(opts) {
  ...
  // v6.598 — DON'T TRAP THE USER ON A SLOW LINK. Firestore's batch.commit() promise stays PENDING
  // (never resolves OR rejects) while the connection is slow/intermittent with offline persistence:
  // the write applies locally at once but the ack only comes back on reconnect. saveConfirmed() then
  // never settles and this full-screen overlay blocks the whole app indefinitely — the "saving takes
  // forever / frozen" report. A genuine failure still rejects fast and takes the red popup below.
  // The change is ALREADY durable on this device (pending-journal + auto-retry keep pushing it), so
  // after a clear stall we stop blocking, apply the change locally, and tell the user it is safe and
  // still syncing — the "unsaved cloud" banner then tracks the real cloud state until it lands.
  const SLOW_SAVE_MS = (opts.slowMs != null) ? opts.slowMs : 10000;
  // The stall path must fire ONLY after real wall-clock time actually elapsed. Test harnesses stub
  // setTimeout to run SYNCHRONOUSLY (0 ms elapsed); without this check the timer would beat every real
  // save and wrongly report a stall. Checking Date.now() keeps production correct (a genuine 10s stall
  // still trips) with no UA sniffing. `opts.__forceStall` lets a test exercise the stall path directly.
  const SLOW_SAVE_MS_N = Number(SLOW_SAVE_MS) || 0;
  const _t0 = Date.now();
  let _slowT = null;
  const res = await Promise.race([
    saveConfirmed().then(r => { if (_slowT) clearTimeout(_slowT); return r; },
                         e => { if (_slowT) clearTimeout(_slowT); return { ok: false, error: (e && (e.code || e.message)) || String(e) }; }),
    new Promise(resolve => { _slowT = setTimeout(() => {
      if (opts.__forceStall || (Date.now() - _t0) >= SLOW_SAVE_MS_N * 0.5) resolve({ ok: false, slow: true });
    }, SLOW_SAVE_MS_N); }),
  ]);
  ...
  if (res && res.slow) {
    ...
    // Apply locally + let the user keep working; the write is journaled and auto-retries in the
    // background, and the header "still saving…" / "Retry now" indicator shows the true cloud state.
    if (typeof opts.onOk === 'function') { try { opts.onOk({ ok: true, slow: true }); } catch (_) {} }
    try { toast('☁ ' + t('Still saving to the cloud — the connection is slow. Your change is safe on this device and keeps syncing automatically; you can keep working.', 'لا يزال الحفظ في السحابة جارياً — الاتصال بطيء. تغييرك محفوظ على هذا الجهاز ويستمر المزامنة تلقائياً؛ يمكنك متابعة العمل.'), 'info'); } catch (_) {}
    if (typeof opts.afterOk === 'function') { try { opts.afterOk(); } catch (_) {} }
    return true;
  }
...
async function load() {
  try {
    ...
    if (state.settings && Array.isArray(state.settings.expenseCategories)) {
      // v6.551: DE-DUPLICATE the selectable list case-insensitively (trimmed). A merge
      // from another device — or an old backup — could seed the same category twice
      // (e.g. "Maintenance" appearing two times in the dropdown). Keep the FIRST
      // occurrence's casing. Expense RECORDS keep their own saved category text, so no
      // expense data is touched — only the picker list is cleaned. Idempotent.
      {
        const _seen = new Set();
        state.settings.expenseCategories = state.settings.expenseCategories
      ...
      // v6.534: add "Products" (stock/goods purchases) if missing, near the operational categories.
      if (!state.settings.expenseCategories.some(c => String(c).toLowerCase() === 'products')) {
        const eqIdx = state.settings.expenseCategories.findIndex(c => String(c).toLowerCase() === 'equipment');
        if (eqIdx >= 0) state.settings.expenseCategories.splice(eqIdx + 1, 0, 'Products');
        else state.settings.expenseCategories.unshift('Products');
      }
      // v6.551: add "Owner Net Profit" (owner take-home / profit distribution) if missing,
      // next to the other owner-money category.
      if (!state.settings.expenseCategories.some(c => String(c).toLowerCase() === 'owner net profit')) {
        const ownIdx = state.settings.expenseCategories.findIndex(c => String(c).toLowerCase() === 'cash collected by owner');
        if (ownIdx >= 0) state.settings.expenseCategories.splice(ownIdx + 1, 0, 'Owner Net Profit');
        else state.settings.expenseCategories.push('Owner Net Profit');
      }
    }
...
function runMigrations(data, fromVersion) {
  ...
  // 9 → 10: introduce the "Mixed" special sport. Add it to state.settings.sports if missing.
  // Additive only — no existing data is touched.
  if (fromVersion < 10) {
    if (!data.settings) data.settings = {};
    if (!Array.isArray(data.settings.sports)) data.settings.sports = [];
    if (!data.settings.sports.some(s => (s.name || s) === MIXED)) {
      const maxOrder = Math.max(0, ...data.settings.sports.map(s => s.order ?? 0));
      data.settings.sports.push({ name: MIXED, enabled: true, order: maxOrder + 1 });
    }
  }
...
// v6.621 — a plausible calendar year/month for the period selectors. A corrupt date like
// "0001-09" (bad manual entry) otherwise seeds a bogus "0001" atop every month/year filter
// and the Reports "Year" tab lands on it. Mirrors the existing Charts/multi-month guards.
function plausibleYearStr(y) { const n = parseInt(y, 10); return n >= 2010 && n <= (new Date().getFullYear() + 1); }
function plausibleMonthKey(m) { return /^\d{4}-\d{2}$/.test(String(m || '')) && plausibleYearStr(String(m).slice(0, 4)); }
...
function availableMonths(opts = {}) {
  ...
  return [...set].filter(plausibleMonthKey).sort();
}
...
function birthdayWaMessage(m) {
  // v6.596 — warmer birthday message: FIRST name in both languages (was the full Arabic name), a
  // sport-neutral "see you at the club" (not "on the mat", odd for swimming/gymnastics), and
  // GENDER-AWARE Arabic (أنتِ/لكِ for a female member) — defaults to masculine when gender is unknown.
  const first = (m && m.name) ? String(m.name).split(' ')[0] : '';
  const firstAr = (m && m.nameArabic) ? String(m.nameArabic).trim().split(/\s+/)[0] : '';
  const female = !!(m && /^f/i.test(String(m.gender || '')) || /أنثى/.test(String(m && m.gender || '')));
  const en = `🎂 Happy Birthday, ${first}! 🎉🎈\n\n`
    + `Wishing you a wonderful day full of joy, health and success — you truly deserve it. The whole Black Stars Sports Club family is celebrating you today! 🖤⭐\n\n`
    + `Enjoy every moment — we can't wait to see you back at the club! 💪`;
  const youWell = female ? 'وأنتِ بخير' : 'وأنت بخير';
  const ya = firstAr ? ' يا ' + firstAr : '';
  const wish = female ? 'نتمنّى لكِ' : 'نتمنّى لك';
  const deserve = female ? 'تستحقّين كل خير' : 'تستحقّ كل خير';
  const celeb = female ? 'تحتفل بكِ' : 'تحتفل بك';
  const enjoy = female ? 'استمتعي بيومكِ، ونراكِ قريباً في النادي' : 'استمتع بيومك، ونراك قريباً في النادي';
  const arMsg = `🎂 كل عام ${youWell}${ya}! 🎉🎈\n\n`
    + `${wish} يوماً جميلاً مليئاً بالفرح والصحة والنجاح — ${deserve}. عائلة نادي بلاك ستارز الرياضي كلها ${celeb} اليوم! 🖤⭐\n\n`
    + `${enjoy}! 💪`;
  return en + '\n\n———\n\n' + arMsg;
}
...
function coachEarnings(coach, monthKey) {
  ...
  for (const inv of (state.invoices || [])) {
    ...
    if (mem && mem.deleted) continue;   // v6.572: an archived member earns nothing (matches computeMonthlyPay / attendance path)
    for (const li of lines) {
      // v6.570 — MIXED: split the line to the coach by classes taught (payment-summary consistency).
      if (li.sport === MIXED) {
        const _share = mixedCoachFeeShare(mem, inv, li, coach.id, null);
        if (_share > 0) { involved = true; coachAmt += _share; eligBase += _share; }
        continue;
      }
      if (String(li.coachId) !== String(coach.id)) continue;   // v6.572: String()-compare — a string coachId (imported 16-digit ids) no longer reads as 0 revenue
      involved = true;
...
// v6.592 — WELCOME-BACK message for a Summer-Camp member (bilingual, Arabic first). The camp is
// seasonal, so instead of "renew your camp" this invites them to the club's YEAR-ROUND activities.
// The activity list is built LIVE from the club's enabled sports (minus the camp + the Mixed package),
// so it stays current — with each sport's emoji and its Arabic name.
function campWinbackMessage(m) {
  const _emoji = (s) => (typeof sportEmoji === 'function') ? sportEmoji(s) : '•';
  const _ar = (s) => (typeof sportNameAR === 'function') ? sportNameAR(s) : s;
  // v6.594 — state.settings.sports holds OBJECTS ({name, enabled, order}), not strings — mapping them
  // directly printed "[object Object]". Normalise to enabled sport NAMES, ordered, minus camp + Mixed.
  const base = (state && state.settings && Array.isArray(state.settings.sports) && state.settings.sports.length)
    ? state.settings.sports
    : (typeof DEFAULT_SPORTS !== 'undefined' ? DEFAULT_SPORTS : []);
  const list = base
    .map(s => (s && typeof s === 'object') ? s : { name: s, enabled: true, order: 0 })
    .filter(s => s && s.name && s.enabled !== false && s.name !== SUMMER_CAMP && (typeof MIXED === 'undefined' || s.name !== MIXED))
    .sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
    .map(s => s.name);
  const enList = list.map(s => `${_emoji(s)} ${s}`).join(' · ');
  const arList = list.map(s => `${_emoji(s)} ${_ar(s)}`).join(' · ');
  const nameEn = m.name || '';
  const nameAr = m.nameArabic || m.name || '';
  const ar = `مرحباً ${nameAr} 👋\n\n`
    + `سعدنا كثيراً بوجودك في المخيم الصيفي بنادي بلاك ستارز! 🌟 أهلاً بك مجدداً في النادي 🖤⭐\n\n`
    + `ولدينا أكثر من نشاط في انتظارك على مدار العام:\n${arList}\n\n`
    + `🎁 عرض خاص لطلاب المخيم: خصم 20% على التايكوندو والكاراتيه!\n\n`
    + `اختر نشاطك المفضّل وانضم إلينا اليوم! 💪\n\n⭐ نادي بلاك ستارز الرياضي`;
  const en = `Hi ${nameEn} 👋\n\n`
    + `It was great having you at our Summer Camp at Black Stars Sports Club! 🌟 Welcome back anytime 🖤⭐\n\n`
    + `And we've got more than one activity waiting for you all year round:\n${enList}\n\n`
    + `🎁 Special offer for camp students: 20% OFF Taekwondo & Karate!\n\n`
    + `Pick your favourite and join us today! 💪\n\n⭐ Black Stars Sports Club`;
  return `${ar}\n\n— — — — — — —\n\n${en}`;
}
if (typeof window !== 'undefined') window.campWinbackMessage = campWinbackMessage;

function buildReminderMessage(m, kind, daysFromToday) {
  // {sport} for the "completed" congratulation must name ONLY the sports the member
  // actually FINISHED (live attendance ≥ class limit) — NOT every enrolled sport, or a
  // member who completed ONE sport is congratulated for all of them (Basil finished
  // Kick Boxing 8/8 but the message claimed Swimming 4/8 + Kick Boxing + Football 5/8).
  // Expired/expiring reminders still list every renewal sport. (v6.552)
  let _sports;
  if (kind === 'completed' && typeof completedSubsForRenewal === 'function') {
    _sports = [...new Set(completedSubsForRenewal(m).map(d => d.sport).filter(Boolean))];
    if (!_sports.length) _sports = memberRenewalSports(m);   // safety net: never blank {sport}
  } else {
    _sports = memberRenewalSports(m);
  }
  // v6.592 — SUMMER CAMP is seasonal, not a year-round membership to "renew". A camp-ONLY member
  // (their only sport is the camp) should get a warm WELCOME-BACK message that invites them to the
  // club's year-round activities, not the generic "renew your Summer Camp" reminder. (Owner request.)
  const _campOnly = _sports.length > 0 && _sports.every(s => (String(s || '').split(' · ')[0]) === SUMMER_CAMP);
  if (_campOnly && typeof campWinbackMessage === 'function') return campWinbackMessage(m);
  ...
  let en = fill(reminderTemplate(enKey), false);
  let ar = fill(reminderTemplate(arKey), true);   // {nameArabic} falls back to {name}

  // v6.584 — an EXPIRED/EXPIRING member who did NOT finish their classes should be told that the
  // unattended classes CARRY OVER when they renew (capped at CARRY_FORWARD_MAX per sport). This never
  // shows on the "completed" congratulation (a finisher has 0 left) or a date-based membership.
  const insBeforeSig = (msg, line) => { const i = msg.lastIndexOf('⭐'); return i >= 0 ? msg.slice(0, i) + line + '\n\n' + msg.slice(i) : msg.replace(/\s*$/, '') + '\n\n' + line; };

  let _carry = 0;
  if (kind !== 'completed') {
    try { _carry = memberRenewalSports(m).reduce((s, sp) => s + (typeof carryForwardCredit === 'function' ? (carryForwardCredit(m, sp) || 0) : 0), 0); } catch (_) {}
  }
  if (_carry > 0) {
    const enLine = `🎁 Good news: you still have ${_carry} ${_carry === 1 ? 'class' : 'classes'} left — renew and we'll carry ${_carry === 1 ? 'it' : 'them'} over to your new package, so nothing is lost 💪`;
    const arLine = `🎁 خبر سعيد: لديك ${_carry === 1 ? 'حصة واحدة متبقية' : _carry === 2 ? 'حصتان متبقيتان' : _carry + ' حصص متبقية'} — جدّد اشتراكك وسنرحّلها إلى باقتك الجديدة، فلن تفقد شيئاً 💪`;
    en = insBeforeSig(en, enLine);
    ar = insBeforeSig(ar, arLine);
  }

  // v6.591 — a "completed" congratulation names only the FINISHED sport(s). When the member is STILL
  // ACTIVE in OTHER sports (finished Summer Camp but mid-way in Karate & Kick Boxing), add a line that
  // acknowledges those ongoing sports, so it doesn't read like they finished everything.
  if (kind === 'completed') {
    const _finishedSet = new Set(_sports);
    let ongoing = [];
    try { ongoing = memberRenewalSports(m).filter(sp => sp && !_finishedSet.has(sp)); } catch (_) {}
    if (ongoing.length) {
      en = insBeforeSig(en, `💪 And you're still going strong in ${joinSports(ongoing, '&')} — keep it up! See you in class 🥋`);
      ar = insBeforeSig(ar, `💪 وما زلت مستمراً بقوة في ${joinSports(ongoing, 'و')} — واصل التألق! نراك في التمرين 🥋`);
    }
  }
...
function invoicePaymentsSumDeduped(inv) {
  ...
  let sum = 0;
  for (const p of pmts) {
    // v6.628 — dedup by the FULL pid, NOT the base. _ensurePaymentIds mints `base#1`/`base#2` for
    // GENUINELY-DISTINCT identical-looking installments (e.g. two 500-cash rows on the same day, no
    // `at`), and the sync merge keys by the FULL pid, so it never fabricates #1/#2 twins for ONE
    // payment. Stripping `#N` collapsed two real installments into one → undercounted paid (phantom
    // balance). Only rows with the SAME full pid are the same payment; those still collapse.
    const key = p && p.pid ? String(p.pid) : null;
    if (key) { if (seen.has(key)) continue; seen.add(key); }   // pid-bearing rows dedup by full pid; pid-less rows always count
    sum += Number(p.amount) || 0;
  }
  return Math.round(sum * 100) / 100;
}
// v6.534 — collapse merge-duplicate payment rows IN PLACE (rows sharing a `pid` base, e.g. "c…#1" and
// "c…#2", which a multi-device merge can create). The payment editors sum rows RAW, so a duplicate row
// doubled the displayed Paid and — worse — was written back into amountPaid on Save, permanently doubling
// recorded money (inv 208 etc.). Removing the extra rows is provably safe: invoicePaid already treats them
// as one, so a member's effective paid/due does not change — only the redundant rows go. Returns true if
// it removed anything. Call this when a payment editor OPENS, so its raw sums are correct.
function dedupeInvoicePayments(inv) {
  if (!inv || !Array.isArray(inv.payments) || inv.payments.length < 2) return false;
  const seen = new Set(); const out = []; let removed = 0;
  for (const p of inv.payments) {
    // v6.628 — dedup by the FULL pid (see invoicePaymentsSumDeduped): `base#1`/`base#2` are DISTINCT
    // installments the merge keeps on purpose, so stripping `#N` deleted real payments and halved
    // amountPaid on Save. Only an exact full-pid duplicate is the same payment.
    const key = p && p.pid ? String(p.pid) : null;
    if (key) { if (seen.has(key)) { removed++; continue; } seen.add(key); }
    out.push(p);
  }
  if (!removed) return false;
  inv.payments = out;
  inv.amountPaid = Math.round(out.reduce((s, p) => s + (Number(p.amount) || 0), 0) * 100) / 100;
  return true;
}
function invoicePaid(inv) {
  ...
  if (led != null) {
    // v6.543: an intentional CORRECTION/REVERSAL in the ledger (a `_correction` row, or any negative
    // amount) is authoritative — it deliberately reduces paid BELOW a possibly-stale cached amountPaid.
    // The old max(amountPaid, led) resurrected the pre-reversal amount, so a fully-reversed payment
    // (e.g. +588 +137 −725, net 0) still read as paid when a device left amountPaid=725 (multi-device):
    // the member showed no UNPAID badge though they owe the full amount (Abdulrahman Ali A Alshahrani).
    // Only trust the ledger when it carries such a correction; otherwise keep the max() guard that
    // protects a merge-clobbered ledger from INCREASING the balance.
    const _hasCorrection = Array.isArray(inv.payments) && inv.payments.some(p => p && (p._correction === true || (Number(p.amount) || 0) < 0));
    return _hasCorrection ? led : Math.max(Number(inv.amountPaid) || 0, led);
  }
...
function billedBySportInPeriod(monthPred) {
  ...
  for (const i of (state.invoices || [])) {
    ...
    for (const li of items) {
      const mo = lineRevenueMonth(li, i);   // v6.574: sport-START month (headline revenue basis) so per-sport bars re-sum to billedInPeriod (was lineBillMonth → didn't tie out)
...
function allDataMonths() {
  ...
  return [...s].filter(plausibleMonthKey);   // drop corrupt "0001"-style years (v6.621)
}
...
// v6.585 — per-subscription PAID, sourced from the INVOICE LINES (the money source of truth) rather
// than the stored sub.amountPaid, which can drift after a switch (e.g. Ezz El-Din's MMA→Riahi line was
// invoiced 262.5 but the sub still read 150, so the card rows summed to 1,725 while the header — which
// is invoice-based — correctly showed 1,838). Returns a Map(subObject → paidAmount). Each invoice line
// is assigned to at most ONE subscription (greedy: exact invoice+sport+coach → sport+coach → sport), so
// a switched or duplicated sport can never double-count. A sub with no matching line (legacy import, or
// an enrollment-only synth row) is omitted → the caller falls back to its stored amountPaid. This makes
// the Subscription-History rows reconcile with the invoice-based header. Money-safe: reads only, no writes.
function memberSubPaidMap(m, allSubs) {
  const map = new Map();
  if (!m) return map;
  const invs = (state.invoices || []).filter(i => !i.deleted && i.customerId === m.id
    && (i.category || 'Membership') === 'Membership' && !i.switchCredit && i.activityType !== 'switch-credit');
  const pool = [];
  for (const inv of invs) {
    const lines = (Array.isArray(inv.lineItems) && inv.lineItems.length) ? inv.lineItems : [{ sport: inv.sport, coachId: inv.coachId, price: inv.amount || 0 }];
    ...
    for (const l of lines) {
      ...
      pool.push({ ref: inv.ref || ('INV' + inv.id), sport: l.sport, coachId: l.coachId, coach: l.coach,
        paid: gross > 0 ? Math.round(price * (paid / gross) * 100) / 100 : price, used: false });
    }
  }
  const _c = v => String(v == null ? '' : v);
  const take = pred => { const e = pool.find(p => !p.used && pred(p)); if (e) { e.used = true; return e.paid; } return null; };
  for (const s of (allSubs || [])) {
    if (!s || !s.activity) continue;
    const ref = s.invoiceNumber; let v = null;
    if (ref) v = take(p => p.ref === ref && p.sport === s.activity && _c(p.coachId) === _c(s.coachId));
    if (v == null && ref) v = take(p => p.ref === ref && p.sport === s.activity);
    if (v == null) v = take(p => p.sport === s.activity && (_c(p.coachId) === _c(s.coachId) || (s.coach && _c(p.coach) === _c(s.coach))));
    if (v == null) v = take(p => p.sport === s.activity);
    if (v != null) map.set(s, v);
  }
  return map;
}
...
// invoices, floored at 0. The old code summed each invoice's balance floored at 0 INDIVIDUALLY, so an
// overpayment on one invoice (e.g. a payment recorded on a switch-credit invoice) was thrown away
// instead of covering another invoice's balance — inflating the due (Layla Karim: 375 vs the true
// 140.63). Netting once, at the member, makes due = charged − paid, which is the simple, stable rule
// (verified on the 2026-08-27 backup: only the one genuinely-stranded member changes; all others equal).
function memberOutstanding(memberId) {
  const invs = (state.invoices || []).filter(i => !i.deleted && i.customerId === memberId && (i.category || 'Membership') === 'Membership');
  let net = 0;
  for (const i of invs) {
    // Legacy invoice (no ledger AND no amountPaid) is fully paid by the app's convention → contributes 0.
    if (i.amountPaid == null && !(Array.isArray(i.payments) && i.payments.length)) continue;
    // Use invoiceTotal (drops a redundant sport-less summary line, v6.441) — NOT the raw stored amount —
    // and do NOT floor per invoice: net the whole member so an overpayment on one invoice covers another.
    net += (invoiceTotal(i) - invoicePaid(i));
  }
  return Math.max(0, Math.round(net * 100) / 100);
}
// The member's total membership PAID (all their membership invoices, incl. any switch-credit) — the
// counterpart to memberOutstanding so a card can show charged / paid / due that always reconcile.
function memberMembershipPaid(memberId) {
  return Math.round((state.invoices || [])
    .filter(i => !i.deleted && i.customerId === memberId && (i.category || 'Membership') === 'Membership')
    .reduce((s, i) => s + invoicePaid(i), 0) * 100) / 100;
}
...
function splitSiblingPayment(members, familyTotal) {
  ...
  sibs.forEach((m, idx) => {
    ...
    if (inv) {
      ...
      // v6.532 (BUG): invoiceTotal sums lineItems and IGNORES inv.amount, so setting only inv.amount to
      // the share left every sibling with a phantom due = fullLinePrice − share. Rescale the line prices
      // so invoiceTotal == the share (each kid then reads charged = paid = share → due 0).
      if (Array.isArray(inv.lineItems) && inv.lineItems.length) {
        const cur = inv.lineItems.reduce((s, li) => s + (Number(li.price) || 0), 0);
        const k = cur > 0 ? myShare / cur : 0;
        let acc = 0;
        inv.lineItems.forEach((li, i) => {
          const v = i === inv.lineItems.length - 1 ? Math.round((myShare - acc) * 100) / 100 : Math.round((Number(li.price) || 0) * k * 100) / 100;
          li.price = v; acc += v;
        });
      }
      ...
      state.invoices.push({
        ...
        // v6.532: scale the enrollment prices to the family share so invoiceTotal == share (no phantom due).
        lineItems: (() => {
          const cur = enr.reduce((s, e) => s + (Number(e.price) || 0), 0);
          const k = cur > 0 ? myShare / cur : 0;
          let acc = 0;
          return enr.map((e, i) => {
            const v = cur > 0 ? (i === enr.length - 1 ? Math.round((myShare - acc) * 100) / 100 : Math.round((Number(e.price) || 0) * k * 100) / 100) : Math.round((myShare / enr.length) * 100) / 100;
            acc += v;
            return { sport: e.sport, coach: coachName(e.coachId), coachId: e.coachId, classes: e.classes, price: v };
          });
        })(),
      });
    }
...
function detectDuplicateInvoices() {
  ...
  for (const inv of usable) {
    ...
    // v6.561: an "exact" (auto-removable) duplicate must be a genuine double-entry — the SAME DATE —
    // for EVERY category, not only rentals/camp. Keying memberships/products by calendar MONTH wrongly
    // flagged a member who joined early in a month and RENEWED at month-end (e.g. 01 Aug + 31 Aug, 30
    // days apart) as an "exact duplicate", so "Remove all exact duplicates" would DELETE the renewal.
    // Same-day double-entries are still caught here; near-but-not-same-day repeats surface in the
    // POSSIBLE tier (≤ 7 days, manual review); monthly renewals (weeks apart) are correctly ignored.
    const period = inv.date || inv.month || '';
  ...
  for (const rows of Object.values(bySig)) {
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      // v6.564: a "possible" duplicate is a near-same-day RE-ENTRY (same customer/items/amount a day or
      // two apart). The window was 7 days, which flagged legitimate WEEKLY renewals/purchases — a member
      // who buys a class or renews every 7 days (Tammim's weekly Football 100, Tallal's weekly KB 120)
      // showed up as a "possible duplicate" even though the dates differ by a full cycle. An accidental
      // double-entry happens same day (→ exact) or next day; anything a week apart is a real renewal.
      if (days <= 2) cluster.push(sorted[i]);
...
// v6.613 — ONLY true DUPLICATES are mergeable: same date + same amount + same sport(s). Two membership
// invoices on DIFFERENT dates are legitimate separate/renewal invoices and must never be flagged
// (the old code flagged any member with 2+ membership invoices → renewals wrongly showed as "merge").
function _invDupKey(inv) {
  const sports = (Array.isArray(inv.lineItems) && inv.lineItems.length)
    ? inv.lineItems.map(li => (li && li.sport) || '')
    : [inv.sport || ''];
  const sportSig = sports.map(s => String(s).trim().toLowerCase()).sort().join(',');
  return `${(inv.date || '').slice(0, 10)}|${Math.round((Number(inv.amount) || 0) * 100)}|${sportSig}`;
}
function findMembersWithMergeableInvoices() {
  ...
  for (const [cid, invs] of Object.entries(byMember)) {
    ...
    // Group by duplicate-key; emit ONE candidate per group of ≥2 genuine duplicates.
    const groups = {};
    for (const iv of invs) { const k = _invDupKey(iv); (groups[k] = groups[k] || []).push(iv); }
    for (const g of Object.values(groups)) {
      if (g.length < 2) continue;   // a single invoice on its own date/amount is NOT a duplicate
      const sorted = g.slice().sort((a, b) => (a.date || '').localeCompare(b.date || ''));
      const total = sorted.reduce((s, i) => s + invoiceTotal(i), 0);
      const paid = sorted.reduce((s, i) => s + invoicePaid(i), 0);
      out.push({ member, invoices: sorted, total, paid, ids: sorted.map(i => i.id) });
    }
  }
  return out;
}
...
function mergeMemberInvoices(memberId, restrictIds) {
  const _restrict = (Array.isArray(restrictIds) && restrictIds.length) ? new Set(restrictIds.map(String)) : null;
  const invs = (state.invoices || []).filter(inv =>
    ...
    && (!_restrict || _restrict.has(String(inv.id))));   // v6.613 — merge only the duplicate group when given
...
// v6.534 — the attendance CELL KEY for a sport, coach-aware. Mirrors the grid's attKeyForSport
// (v6.509): when a member holds the SAME sport under 2+ ACTIVE coaches at once, the first coach keeps
// the plain `sport` key and each additional coach writes `sport + ' ' + coachId`. The salary path used
// to read only the plain key, so BOTH coaches were paid for the first coach's marks (Ezz El-Din
// double-pay). Passing the sub's coach here credits each coach for the cell HIS row actually wrote.
function attendanceKeyFor(m, sport, coachId) {
  if (coachId == null || !m) return sport;
  ...
  if (coaches.length < 2) return sport;                                   // single active coach → shared key
  return String(coachId) === coaches[0] ? sport : sport + ' ' + coachId;  // extra coaches → own cell
}
// v6.571 — DOUBLE-PAY GUARD for the PLAIN shared cell. When a member holds the same sport under several
// coaches but only ONE is currently active (e.g. a completed/switched-away HISTORY coach + a LIVE coach),
// attendanceKeyFor funnels BOTH into the plain `sport` cell — so the commission path counted the same
// physical class for EACH coach (Basil: Swimming under Mostafa + Zakaria → one class paid twice). These
// helpers decide which single coach OWNS a given day of the plain cell, so a day is paid once.
// Owner = among the subs that funnel to the plain cell and whose attendance window covers the day, prefer
// an ACTIVE (non-finished) sub, then the latest start. Two SIMULTANEOUSLY-active coaches are unaffected —
// the extra coach has its own `sport <id>` cell and never funnels to plain.
function _finishedSub(s) { const st = (s.status || '').toLowerCase(); return st === 'completed' || st === 'withdrawn' || !!s.switchedAwayTo; }
function plainCellDayOwner(m, sport, dayISO) {
  const subs = (m.subscriptions || []).filter(s => (s.activity || '') === sport && s.coachId != null
    && attendanceKeyFor(m, sport, s.coachId) === sport);
  if (subs.length <= 1) return subs[0] ? String(subs[0].coachId) : null;
  const win = s => (typeof subAttendanceWindow === 'function') ? subAttendanceWindow(m, s) : { from: s.start || null, to: s.end || null };
  const cover = subs.map(s => ({ s, w: win(s) })).filter(x => (!x.w.from || dayISO >= x.w.from) && (!x.w.to || dayISO <= x.w.to));
  const pool = cover.length ? cover : subs.map(s => ({ s, w: win(s) }));
  pool.sort((a, b) => { const af = _finishedSub(a.s) ? 1 : 0, bf = _finishedSub(b.s) ? 1 : 0; if (af !== bf) return af - bf; return String(b.s.start || '').localeCompare(String(a.s.start || '')); });
  return String(pool[0].s.coachId);
}
// Count the plain-cell Y days OWNED by coachId, within [from,to], optionally ≤ uptoDate and/or in monthKey.
function plainCellAttended(m, sport, coachId, from, to, uptoDate, monthKey) {
  const da = m && m.dailyAttendance; if (!da) return 0; let n = 0;
  for (const mo of Object.keys(da)) {
    if (monthKey && mo !== monthKey) continue;
    const cell = (da[mo] && da[mo][sport]) || {};
    for (const d of Object.keys(cell)) {
      if (cell[d] !== 'Y') continue;
      const iso = `${mo}-${String(parseInt(d, 10)).padStart(2, '0')}`;
      if (from && iso < from) continue; if (to && iso > to) continue; if (uptoDate && iso > uptoDate) continue;
      if (String(plainCellDayOwner(m, sport, iso)) === String(coachId)) n++;
    }
  }
  return n;
}
// True when THIS sub shares the plain cell with another same-sport sub (so ownership arbitration applies).
function plainCellShared(m, sport, coachId) {
  if (!m || (typeof attendanceKeyFor === 'function' && attendanceKeyFor(m, sport, coachId) !== sport)) return false;
  return (m.subscriptions || []).filter(s => (s.activity || '') === sport && s.coachId != null
    && (typeof attendanceKeyFor === 'function' ? attendanceKeyFor(m, sport, s.coachId) : sport) === sport).length > 1;
}
function attendanceFor(m, monthKey, sport, coachId) {
  const mo = m?.dailyAttendance?.[monthKey];
  if (!mo) return {};
  // Per-sport: values are objects keyed by day
  const sample = Object.values(mo)[0];
  if (sample && typeof sample === 'object') {
    return mo[attendanceKeyFor(m, sport, coachId)] || {};
  }
...
function liveAttendanceCount(m, sport = null, fromDate = null, toDate = null) {
  ...
  if (da) for (const monthKey of Object.keys(da)) {
  ...
  // v6.571 — a Mixed package's attendance lives in m.mixedAttendance (per-day sport+coach), NOT
  // dailyAttendance. Include it under the Mixed sport (and in the all-sports total) so completion,
  // Ready-to-Renew, member status and the card all see a Mixed member's real attendance.
  if ((sport === null || sport === MIXED) && m && m.mixedAttendance) {
    for (const monthKey of Object.keys(m.mixedAttendance)) {
      const days = m.mixedAttendance[monthKey] || {};
      for (const [dayKey, rec] of Object.entries(days)) {
        if (!rec || rec.mark === 'N') continue;
        if (!inWindow(monthKey, dayKey)) continue;
        y++;
      }
    }
...
// ── MIXED sport helpers (v6.570) ─────────────────────────────────────────────
// A "Mixed" package is ONE invoice line / sub (no single coach). Each attended class is recorded with
// the ACTUAL sport + coach that day in m.mixedAttendance[monthKey][day] = { coachId, sport, mark:'Y' }.
// Commission is split to the real coach: each coach earns (Mixed price / total classes) × the classes
// they personally taught. These helpers are the ONLY readers of that structure. All gated on MIXED so
// no existing sport is affected.
function mixedAttendedByCoach(m, coachId, fromDate, toDate) {
  const ma = m && m.mixedAttendance; if (!ma) return 0;
  let n = 0;
  for (const mo of Object.keys(ma)) {
    const days = ma[mo] || {};
    for (const dk of Object.keys(days)) {
      const rec = days[dk];
      if (!rec || rec.mark === 'N') continue;                       // only attended (Y) classes pay
      if (coachId != null && String(rec.coachId) !== String(coachId)) continue;
      const full = `${mo}-${String(parseInt(dk, 10)).padStart(2, '0')}`;
      if (fromDate && full < fromDate) continue;
      if (toDate && full > toDate) continue;
      n++;
    }
  }
  return n;
}
// Attended Mixed classes by a coach WITHIN one month (still honouring the package window).
function mixedAttendedByCoachInMonth(m, coachId, monthKey, fromDate, toDate) {
  const days = (m && m.mixedAttendance && m.mixedAttendance[monthKey]) || {};
  let n = 0;
  for (const dk of Object.keys(days)) {
    const rec = days[dk];
    if (!rec || rec.mark === 'N') continue;
    if (coachId != null && String(rec.coachId) !== String(coachId)) continue;
    const full = `${monthKey}-${String(parseInt(dk, 10)).padStart(2, '0')}`;
    if (fromDate && full < fromDate) continue;
    if (toDate && full > toDate) continue;
    n++;
  }
...
// Total Mixed package classes (from the sub, else the enrollment) — the divisor for per-class value.
function mixedTotalClasses(mem, inv, li) {
  const sub = (typeof findSubForLine === 'function') ? findSubForLine(mem, inv, li) : null;
  let t = (sub && parseFloat(sub.totalClasses)) || 0;
  if (t <= 0 && mem && Array.isArray(mem.enrollments)) {
    const e = mem.enrollments.find(e => (e.sport || '') === MIXED && (parseInt(e.classes) || 0) > 0);
    if (e) t = parseInt(e.classes) || 0;
  }
  return t;
}
// The Mixed package's attendance window {from,to} (reuses subAttendanceWindow when a sub exists).
function mixedWindow(mem, inv, li) {
  const sub = (typeof findSubForLine === 'function') ? findSubForLine(mem, inv, li) : null;
  if (sub && typeof subAttendanceWindow === 'function') return subAttendanceWindow(mem, sub);
  return { from: (sub && sub.start) || inv.date || null, to: (sub && sub.end) || null };
}
// A coach's FEE share of a Mixed line = perClass × classes they taught (used by payment-basis paths).
function mixedCoachFeeShare(mem, inv, li, coachId, uptoDate) {
  const tot = mixedTotalClasses(mem, inv, li); if (tot <= 0) return 0;
  const taught = mixedCappedByCoach(mem, inv, li, coachId, uptoDate || null, null);   // v6.571: capped at paid classes
  return (parseFloat(li.price) || 0) / tot * taught;
}
// Every coach who taught at least one Mixed class in the package window (for report fan-out).
function mixedCoachesTaught(mem, inv, li, uptoDate) {
  const w = mixedWindow(mem, inv, li);
  const to = (uptoDate && (!w.to || uptoDate < w.to)) ? uptoDate : w.to;
  const ma = mem && mem.mixedAttendance; const set = new Set();
  if (ma) for (const mo of Object.keys(ma)) { const days = ma[mo] || {};
    for (const dk of Object.keys(days)) { const rec = days[dk]; if (!rec || rec.mark === 'N' || rec.coachId == null) continue;
      const full = `${mo}-${String(parseInt(dk, 10)).padStart(2, '0')}`;
      if (w.from && full < w.from) continue; if (to && full > to) continue; set.add(String(rec.coachId)); } }
  return [...set];
}
// v6.571 — the attended Mixed classes in DATE order within the package window, CAPPED at totalClasses.
// Returns [{ date, coachId }]. Every COMMISSION path counts through this so the per-coach split can never
// exceed the package fee even if the member is over-marked (attended > paid classes). Display paths keep
// the raw count (mixedAttendedByCoach) so the card still shows the true attendance.
function mixedCappedEntries(mem, inv, li) {
  const tot = mixedTotalClasses(mem, inv, li); if (tot <= 0) return [];
  const w = mixedWindow(mem, inv, li);
  const ma = mem && mem.mixedAttendance; const rows = [];
  if (ma) for (const mo of Object.keys(ma)) { const days = ma[mo] || {};
    for (const dk of Object.keys(days)) { const rec = days[dk]; if (!rec || rec.mark === 'N') continue;
      const iso = `${mo}-${String(parseInt(dk, 10)).padStart(2, '0')}`;
      if (w.from && iso < w.from) continue; if (w.to && iso > w.to) continue;
      rows.push({ date: iso, coachId: rec.coachId }); } }
  rows.sort((a, b) => a.date.localeCompare(b.date));
  return rows.slice(0, tot);   // only the first `totalClasses` paid slots earn commission
}
// Capped count for one coach, optionally up to a settle date and/or within one month.
function mixedCappedByCoach(mem, inv, li, coachId, uptoDate, monthKey) {
  return mixedCappedEntries(mem, inv, li).filter(e =>
    String(e.coachId) === String(coachId)
    && (!uptoDate || e.date <= uptoDate)
    && (!monthKey || e.date.slice(0, 7) === monthKey)).length;
}
...
function isCompleted(m) {
  ...
  for (const s of current) {
    ...
    let attended = s.attendedClasses;
    // v6.571 — a Mixed package's attendance lives in m.mixedAttendance, so the stored attendedClasses
    // stays 0; read the live mixed count instead, else a finished Mixed member never reads Completed.
    if (s.activity === MIXED && typeof mixedAttendedByCoach === 'function') {
      ...
      attended = mixedAttendedByCoach(m, null, w.from, w.to);
    }
...
function attendedYInMonth(m, sport, monthKey, startDate, endDate, uptoDate, coachId) {
  const day = attendanceFor(m, monthKey, sport, coachId) || {};
  let y = 0;
...
function attendedYForSub(m, sub, uptoDate) {
  ...
  let y = 0;
  // v6.534: pass the sub's coach so a same-sport two-coach member counts only THIS coach's cell.
  for (const monthKey of Object.keys(da)) y += attendedYInMonth(m, sub.activity, monthKey, w.from, w.to, uptoDate, sub.coachId);
  return y;
}
...
function findSubForLine(m, inv, li) {
  ...
  // v6.562: the coach-blind ref fallback must ONLY apply when the LINE itself has no coach. When a line
  // names a coach, it must NEVER resolve to a DIFFERENT coach's subscription — otherwise a stray line for
  // coach B on an invoice whose sub belongs to coach A (e.g. a phantom Aziz line left on Abdel Salam's
  // original invoice by an old switch) credits B for A's attended classes. With a coach on the line and
  // no ref+coach match, we fall through to the coach-filtered candidates below (its own sub), never A's.
  const byRef = subs.find(s => s.invoiceNumber === inv.ref && s.activity === li.sport && (li.coachId == null || String(s.coachId) === String(li.coachId)))
    || (li.coachId == null ? subs.find(s => s.invoiceNumber === inv.ref && s.activity === li.sport) : null);
...
// v6.567: SWITCH SAFETY NET. A sport switch has several cycle-selection sites; a bad pick can write a
// STRUCTURALLY BROKEN subscription — a backwards window (start after end) or a zero-day destination
// window — which then reads as "expired", blocks attendance marking, and mis-pays commission (Basil's
// Swimming: Zakaria 12 Sep→12 Sep). This validator inspects the RESULT of a switch and returns a plain
// reason if any ACTIVE subscription is broken; the switch handler snapshots state before mutating and
// ROLLS BACK (nothing saved) when this returns non-null — so even a NEW edge case fails safe instead of
// corrupting the data. Kept intentionally narrow (only unambiguously-broken windows) → no false blocks.
function switchResultProblem(m) {
  const subs = (m && m.subscriptions) || [];
  for (const s of subs) {
    if (!s || s.status === 'completed' || s.status === 'withdrawn' || s.switchedAwayTo) continue;
    if (s.start && s.end) {
      if (String(s.start) > String(s.end)) return 'a ' + (s.activity || 'sport') + ' subscription would start after it ends';
      if (s.switchFunded && String(s.start) === String(s.end)) return 'the new ' + (s.activity || 'sport') + ' subscription would have a zero-day window';
    }
  }
  return null;
}
if (typeof window !== 'undefined') window.switchResultProblem = switchResultProblem;
...
function computeMemberCommissions(ym) {
  ...
  let invs;
  if (all) {
    invs = state.invoices.filter(i => (i.category || 'Membership') === 'Membership' && !i.deleted);
  } else {
    invs = monthInvoicesAny(ym).filter(i => (i.category || 'Membership') === 'Membership');   // line-month aware: include multi-month invoices
    // v6.572 — a MIXED package's classes can be ATTENDED in a month other than the invoice's bill month.
    // monthInvoicesAny keys off bill/line month, so it would miss such an invoice; add any Mixed invoice
    // whose member attended a Mixed class in `ym` (the per-attended-month fan-out below scopes the rows).
    const _seen = new Set(invs.map(i => i.id));
    for (const inv of state.invoices) {
      if (inv.deleted || _seen.has(inv.id)) continue;
      if ((inv.category || 'Membership') !== 'Membership') continue;
      if (!(Array.isArray(inv.lineItems) && inv.lineItems.some(li => li.sport === MIXED))) continue;
      const mem = inv.customerId ? state.members.find(x => x.id === inv.customerId) : null;
      if (!mem || !mem.mixedAttendance || !mem.mixedAttendance[ym]) continue;
      if (Object.values(mem.mixedAttendance[ym]).some(r => r && r.mark !== 'N')) { invs.push(inv); _seen.add(inv.id); }
    }
  }
  const rows = [];
  for (const inv of invs) {
    ...
    const lineItems = commissionLineItems(inv, mem);
    for (const li of lineItems) {
      // v6.570/6.572 — MIXED: fan the single Mixed line out into ONE row PER coach PER ATTENDED MONTH,
      // each crediting (Mixed price / total classes) × the classes that coach taught THAT month — so the
      // Member-Commission report reconciles with the month-aware Salaries screen. NOTE: handled BEFORE the
      // bill-month gate below, because a Mixed package billed in July but attended in August must still
      // show under August (the gate would otherwise drop it). Capped at the paid classes (v6.571).
      if (li.sport === MIXED) {
        const _tot = mixedTotalClasses(mem, inv, li);
        if (_tot <= 0) continue;
        const _perClass = (parseFloat(li.price) || 0) / _tot;
        const _sub = mem ? findSubForLine(mem, inv, li) : null;
        const _groups = {};   // `${coachId}|${YYYY-MM}` → classes taught by that coach that month
        for (const e of mixedCappedEntries(mem, inv, li)) {
          const _mo = String(e.date).slice(0, 7);
          if (!all && _mo !== ym) continue;                 // scope to the requested month by ATTENDED date
          if (e.coachId == null) continue;
          const _k = String(e.coachId) + '|' + _mo; _groups[_k] = (_groups[_k] || 0) + 1;
        }
        for (const _k of Object.keys(_groups)) {
          const _sep = _k.lastIndexOf('|'); const _cid = _k.slice(0, _sep); const _mo = _k.slice(_sep + 1);
          const _taught = _groups[_k]; if (_taught <= 0) continue;
          const _coach = state.coaches.find(c => String(c.id) === String(_cid));
          const _rate = _coach ? (parseFloat(_coach.rate) || 0) : 0;
          rows.push({
            invoiceId: inv.id, ref: inv.ref || '', month: _mo,
            memberId: mem ? mem.id : null, memberName: mem ? (mem.name || '') : (inv.customerName || '—'),
            nameArabic: mem ? (mem.nameArabic || '') : '', sport: MIXED,
            coachId: _coach ? _coach.id : _cid, coachName: (typeof coachName === 'function') ? coachName(_coach ? _coach.id : _cid) : String(_cid),
            start: (_sub && _sub.start) || inv.date || (inv.month ? inv.month + '-01' : ''),
            expiry: (mem && mem.expiryDate) || (_sub && _sub.end) || '',
            paid: _perClass * _taught, attended: _taught, total: _tot, status: mem ? memberStatus(mem) : '—',
            mode: 'mixed', ratio: _tot > 0 ? _taught / _tot : 0, commissionBase: _perClass * _taught,
            rate: _rate, commission: _perClass * _taught * _rate / 100,
          });
        }
        continue;
      }
  ...
  // v6.626 — collapse GENUINE duplicate invoices so this report reconciles with payroll (which drops
  // duplicate pay via computeAttendanceCommission's _collapseDupes). Two rows matching on
  // member+sport+coach+month+period+amount come from a duplicated membership invoice → credit once.
  // The key includes coachId + month so a legit Mixed per-coach/per-month fan-out, a same-sport-two-
  // coaches enrolment, and a renewal (different start/expiry) are NOT collapsed.
  const _seenDup = new Set();
  const deduped = [];
  for (const r of rows) {
    const _amt = Math.round((Number(r.paid) || 0) * 100);
    const _k = [r.memberId == null ? '' : r.memberId, r.sport || '', r.coachId == null ? '' : r.coachId, r.month || '', r.start || '', r.expiry || '', _amt].join('|');
    if (_seenDup.has(_k)) continue;   // duplicate invoice — already credited once above
    _seenDup.add(_k); deduped.push(r);
  }
  deduped.sort((a, b) =>
...
function computeAttendanceCommission(coachId, monthKey, uptoDate) {
  ...
  // v6.535 — optional "pay attended only": when ON (per-coach flag OR the club-wide setting), an
  // EXPIRED membership does NOT true-up the unattended remainder — the coach is paid the NET ATTENDED
  // amount only. Carried-forward/unused classes earn nothing at expiry. Off by default (keeps true-up).
  const _cForPay = (state.coaches || []).find(x => String(x.id) === String(coachId));
  const attendedOnly = !!(_cForPay && _cForPay.payAttendedOnly) || !!(state.settings && state.settings.payAttendedOnly);
  for (const inv of state.invoices) {
    ...
    const lineItems = commissionLineItems(inv, mem);
    for (const li of lineItems) {
      // v6.570 — MIXED package: one line (no single coach); commission is SPLIT to the coach who actually
      // taught each attended class. Credit THIS coach (Mixed price / total classes) × the classes THEY
      // taught in the window/month. Attendance-driven by nature — runs for a Mixed line regardless of the
      // coach's basis. No pending / no true-up (unattended Mixed classes pay nobody). Month-aware so the
      // Salaries screen and the revenue report agree.
      if (li.sport === MIXED) {
        if (mem && isExcludedFromCoachSalary(coachId, mem.id)) continue;
        const _tot = mixedTotalClasses(mem, inv, li);
        if (_tot <= 0) continue;                                    // can't size the split → no false pay
        const _sub0 = findSubForLine(mem, inv, li);
        const _anchor = (_sub0 && _sub0.start) || inv.date || inv.month || '';
        if (commStart && _anchor && String(_anchor).slice(0, 10) < commStart) continue;
        const _perClass = (parseFloat(li.price) || 0) / _tot;
        // v6.571: count through the CAP (first `totalClasses` attended, date-ordered) so the per-coach
        // split can never exceed the package fee even when the member is over-marked.
        const _taught = uptoDate
          ? mixedCappedByCoach(mem, inv, li, coachId, uptoDate, null)
          : mixedCappedByCoach(mem, inv, li, coachId, null, monthKey);
        if (_taught > 0) {
          base += _perClass * _taught;
          lines.push({ memberName: mem ? mem.name : (inv.customerName || '— deleted —'),
            mid: mem ? ('m' + mem.id) : (inv.customerId != null ? 'c' + inv.customerId : 'n:' + (inv.customerName || '')),
            sport: MIXED, kind: 'attended', classes: _taught, perClass: _perClass, amountBase: _perClass * _taught,
            start: _sub0 && _sub0.start, end: _sub0 && _sub0.end, attended: _taught, total: _tot,
            status: mem ? memberStatus(mem) : '—', note: 'Mixed — classes taught by this coach' });
        }
        continue;
      }
      ...
      // v6.545 (SWITCH DOUBLE-PAY FIX): a switch-credit's POSITIVE line pays the NEW coach their
      // transferred share. But some switches (old net-zero-credit model, later reconciled to the v6.520
      // split-in-place model) recorded that SAME transferred package BOTH on the switch-credit AND as a
      // `switchFunded` subscription on the member's real invoice — so the new coach was paid TWICE (once
      // attendance-based on the real switchFunded line, once flat on the credit line). Layla's Taekwondo
      // sits on the real INV986589 (switchFunded sub) AND the credit SW-923783 → Ahmed billed twice.
      // If the member has a `switchFunded` subscription for THIS sport+coach, the destination is already
      // paid via the real line → the credit line is redundant → skip it. This is precise: it does NOT
      // touch an OLD-model switch whose destination lives ONLY on the credit (no switchFunded sub), nor a
      // switch that adds transferred value ON TOP of an ORIGINAL package (that sub isn't switchFunded).
      if ((inv.switchCredit || inv.activityType === 'switch-credit') && fee > 0 && mem
        && Array.isArray(mem.subscriptions)
        && mem.subscriptions.some(s => (s.activity || '') === li.sport && String(s.coachId) === String(li.coachId) && s.switchFunded)) continue;
      ...
      // v6.571 — DOUBLE-PAY GUARD: when this sport's marks funnel into the plain shared cell for 2+ coaches
      // (a history coach + a live coach), count only the days THIS coach owns (plainCellDayOwner), so one
      // physical class is never paid to two coaches. Normal single-coach + two-active-coach cases are
      // untouched (plainCellShared is false there).
      const _pShared = !!(mem && sub && plainCellShared(mem, li.sport, sub.coachId));
      const _pWin = _pShared ? ((typeof subAttendanceWindow === 'function') ? subAttendanceWindow(mem, sub) : { from: sub.start, to: sub.end }) : null;

      if (uptoDate) {
        // ── SETTLEMENT (cumulative): everything earned from the start through the date ──
        const attended = _pShared ? plainCellAttended(mem, li.sport, sub.coachId, _pWin.from, _pWin.to, uptoDate, null) : attendedYForSub(mem, sub, uptoDate);
        ...
        if (!attendedOnly && ended && remaining > 0 && attended > 0 && !settledMonth && !switchedAway) {   // expired WITH ≥1 attended → pay the rest in full (unless "pay attended only")
        ...
        const attMonth = _pShared ? plainCellAttended(mem, li.sport, sub.coachId, _cw.from, _cw.to, null, monthKey) : attendedYInMonth(mem, li.sport, monthKey, _cw.from, _cw.to, undefined, sub.coachId);   // v6.534 coach-aware; v6.571 plain-cell owner
        if (attMonth > 0) {
        ...
        const attendedAll = _pShared ? plainCellAttended(mem, li.sport, sub.coachId, _cw.from, _cw.to, null, null) : attendedYForSub(mem, sub);
        const remaining = Math.max(0, totalClasses - attendedAll);
        if (!attendedOnly && endMonth === monthKey && ended && remaining > 0 && attendedAll > 0 && !settledMonth && !switchedAway) {   // v6.445: only if ≥1 attended; skip when "pay attended only"
          base += perClass * remaining;
          lines.push({ memberName, mid, sport: li.sport, kind: 'trueup', classes: remaining, perClass,
...
function computeMonthlyPay(coachId, monthKey, uptoDate) {
  ...
  if (basis === 'attendance') {
    ...
    for (const inv of state.invoices) {
      ...
      for (const li of lineItems) {
        ...
        totalFee += price;
        if (li.sport === SUMMER_CAMP) continue;                       // camp earns no commission
        // v6.570 — MIXED: no single coach on the line; credit THIS coach the fraction of the Mixed fee
        // that matches the classes THEY taught, so payment basis splits the same way attendance basis does.
        if (li.sport === MIXED) {
          if (isExcludedFromCoachSalary(coachId, inv.customerId)) continue;
          coachFee += mixedCoachFeeShare(memForLine, inv, li, coachId, uptoDate);
          continue;
        }
        if (String(li.coachId) !== String(coachId)) continue;
        if (isExcludedFromCoachSalary(coachId, inv.customerId)) continue;
      ...
      // v6.532: many invoices store amountPaid with NO payments[] ledger (a common create/edit path).
      // The old loop read inv.payments only, so a payment-basis coach earned 0 on those fully-paid
      // invoices (Aziz Private under-paid ~2,736). Fall back to a synthetic payment from amountPaid,
      // dated at the invoice, so the money is credited in the invoice's month.
      let _pays = (Array.isArray(inv.payments) ? inv.payments : []).filter(p => (Number(p.amount) || 0) > 0);
      if (!_pays.length && (Number(inv.amountPaid) || 0) > 0) {
        _pays = [{ amount: Number(inv.amountPaid), date: inv.date || (inv.month ? inv.month + '-01' : ''), month: inv.month }];
      }
      // v6.555 — OWNER RULE: a "by payment / full amount" coach earns their % of what's been PAID,
      // credited in the invoice's BILLING (enrolment) month — NOT split by the month each payment
      // happened to land in. So a membership billed in Aug but paid partly on 1 Sep still pays the
      // coach entirely in Aug (no carry-forward across months). Unpaid amounts still earn nothing,
      // so a part-paid membership is not over-paid. (Was: each payment counted in its OWN date's
      // month, which split one membership's commission across two months — Aziz Private: 72 in Aug
      // + 792 in Sep for one Aug membership.)
      const billMonth = (typeof invoiceBillMonth === 'function')
        ? invoiceBillMonth(inv) : (inv.month || String(inv.date || '').slice(0, 7));
      if (!uptoDate && monthKey && billMonth !== monthKey) continue;   // this membership's billing month only
      for (const p of _pays) {
        ...
        if (uptoDate && (!pDate || String(pDate).slice(0, 10) > uptoDate)) continue;   // cumulative up to the settle date
  ...
  // v6.560: once a coach is PAID, if his commission GROWS afterwards (a new customer/payment booked to
  // this month after payment), the month must show PARTIALLY PAID with the new remaining — not stay
  // "Paid". So the target tracks the LIVE net: max(agreed target, current net). A later DROP in net
  // still can't claw the settled amount back below what was agreed (the higher target wins). This makes
  // "salary increases automatically + status → partial + remaining shown" work for both bases.
  const paidTarget = paidRecord ? Math.max(salaryTarget(paidRecord, net), net) : net;
...
function audit(action, target, summary, details = null) {
  // v6.580 — the audit log is OFF by default and opt-in via Settings (state.settings.auditEnabled).
  // When disabled we record nothing, so the log never grows (it was ~50% of total storage and the
  // cause of the localStorage-quota warnings) and never reaches localStorage or Firestore.
  if (!(state.settings && state.settings.auditEnabled)) return;
...
function loginScreen() {
  ...
  const doLogin = async () => {
    ...
    try {
      ...
      try { recordLogin(state.user); } catch (_) {}   // v6.611 — capture the sign-in (Logins Logs Audit)
      render();
...
// ─── Login capture (Logins Logs Audit, v6.611) ───────────────────────────────
// Append one row per explicit sign-in to the synced `loginLogs` collection. Bounded to the last
// 3000 rows. Non-fatal — a logging failure must never block a sign-in.
function recordLogin(u) {
  try {
    if (!u || !u.email) return;
    if (!Array.isArray(state.loginLogs)) state.loginLogs = [];
    let device = '';
    try { device = (state.settings && state.settings.deviceName) || localStorage.getItem('bs-device-name') || ''; } catch (_) {}
    const row = {
      id: (typeof nextId === 'function') ? nextId(state.loginLogs) : (Date.now() + Math.floor(Math.random() * 1000)),
      email: u.email, name: u.name || '', role: u.role || '',
      at: new Date().toISOString(),
      device: device || '',
      ua: (typeof navigator !== 'undefined' && navigator.userAgent) ? String(navigator.userAgent).slice(0, 180) : '',
    };
    state.loginLogs.push(row);
    if (state.loginLogs.length > 3000) state.loginLogs = state.loginLogs.slice(-3000);
    if (typeof save === 'function') save();
  } catch (_) {}
}
if (typeof window !== 'undefined') window.recordLogin = recordLogin;
...
const ROUTES = {
  ...
  charts:     { label: 'Charts',     icon: '📊', section: 'Main', adminOnly: false, hidden: true },   // v6.574 merged into Report (Visual insights); route kept for deep-links
  ...
  social:     { label: 'Social Media', icon: '🌐', section: 'Engagement', adminOnly: true, hidden: true },   // v6.472 — compose once, share everywhere. v6.597: hidden per owner (route kept for deep-links)
  ...
  transactions:{ label: 'Transactions', icon: '🧾', section: 'Main' },
  ...
  audit:      { label: 'Audit Log',  icon: '📋', section: 'System', adminOnly: true, get hidden() { return !(typeof state !== 'undefined' && state.settings && state.settings.auditEnabled); } },   // v6.580 — off by default, opt-in in Settings
  ...
  loginlogs:  { label: 'Logins Logs Audit', icon: '🕵', section: 'System', adminOnly: true },   // v6.611 — every sign-in captured
...
function navigate(route, opts) {
  ...
  // v6.625 — the whole System module is PIN-gated; re-lock whenever we leave the System section so
  // it re-prompts on the next visit.
  if (!isSystemRoute(route)) { try { window._systemUnlocked = false; } catch (_) {} }
...
// ─── Render ──────────────────────────────────────────────────────────
// v6.539: gentle "data changed elsewhere" pill for the big working grids (Attendance / Schedule) that
// deliberately do NOT auto-repaint on a remote sync (see NO_AUTO_REPAINT_ROUTES) — otherwise a payment or
// status changed on ANOTHER device leaves e.g. an UNPAID badge stale until the user happens to act. Rather
// than a disruptive full rebuild of the grid (the "why did it refresh suddenly" complaint that disabled the
// auto-repaint), we surface a one-tap refresh pill. Any real render() clears it (the view is then fresh).
function _showSyncRefreshPill() {
  try {
    if (typeof document === 'undefined' || !document.body) return;
    if (document.getElementById('sync-refresh-pill')) return;
    const el = document.createElement('div');
    el.id = 'sync-refresh-pill';
    el.setAttribute('role', 'button');
    el.style.cssText = 'position:fixed;bottom:18px;left:50%;transform:translateX(-50%);z-index:9998;background:var(--accent-2,#f59e0b);color:#fff;font-weight:700;font-size:13px;padding:9px 16px;border-radius:999px;box-shadow:0 6px 18px rgba(0,0,0,.28);cursor:pointer;display:flex;align-items:center;gap:8px;max-width:92vw';
    el.textContent = '🔄 ' + (typeof t === 'function' ? t('Data changed on another device — tap to refresh', 'تغيّرت البيانات على جهاز آخر — اضغط للتحديث') : 'Data changed — tap to refresh');
    el.addEventListener('click', () => { _hideSyncRefreshPill(); try { render(); } catch (_) {} });
    document.body.appendChild(el);
  } catch (_) {}
}
function _hideSyncRefreshPill() { try { const e = document.getElementById('sync-refresh-pill'); if (e) e.remove(); } catch (_) {} }

// ─── System-module PIN gate (v6.625) ────────────────────────────────
// Every page in the System section (Users & Roles, Preferences, Club Setup, Data & Backup,
// Cleanup, Trash, Audit Log, Login Logs, Data Import/Export, Danger Zone, Settings) sits behind
// a PIN ('4242'). ONE unlock covers the whole section while you stay inside it; navigating to any
// non-System page re-locks it (see navigate()). Client-side gate only — NOT cryptographic security.
const SYSTEM_PIN = '4242';
function isSystemRoute(route) { return !!(ROUTES[route] && ROUTES[route].section === 'System'); }
function systemPinScreen(main, route) {
  const label = (ROUTES[route] && ROUTES[route].label) || t('System', 'النظام');
  main.innerHTML = `
    <div class="topbar"><div><h1>🔒 ${t('System', 'النظام')}</h1><div class="subtitle">${escapeHtml(label)} · ${t('Enter the PIN to continue', 'أدخل الرمز للمتابعة')}</div></div></div>
    <div class="card" style="max-width:360px;margin:48px auto;text-align:center;padding:28px;border:1px solid var(--border)">
      <div style="font-size:42px">🔒</div>
      <div style="font-weight:800;font-size:16px;margin:8px 0 4px">${t('Protected system page', 'صفحة نظام محمية')}</div>
      <div class="text-mute" style="font-size:12px;margin-bottom:18px;line-height:1.5">${t('System pages change core settings and data. Enter the PIN to open them.', 'صفحات النظام تُغيّر الإعدادات والبيانات الأساسية. أدخل الرمز لفتحها.')}</div>
      <input id="system-pin" type="password" inputmode="numeric" autocomplete="off" maxlength="8" placeholder="••••" style="width:150px;text-align:center;font-size:24px;letter-spacing:8px;padding:10px;border:1.5px solid var(--border);border-radius:10px;background:var(--surface);color:var(--text)" onkeydown="if(event.key==='Enter')window._systemUnlock()" />
      <div style="margin-top:18px;display:flex;gap:8px;justify-content:center">
        <button class="btn ghost" onclick="navigate('dashboard')">${t('Cancel', 'إلغاء')}</button>
        <button class="btn primary" onclick="window._systemUnlock()">🔓 ${t('Unlock', 'فتح')}</button>
      </div>
    </div>`;
  setTimeout(() => { try { const el = document.getElementById('system-pin'); if (el) el.focus(); } catch (_) {} }, 0);
}
window._systemUnlock = function() {
  const el = document.getElementById('system-pin');
  const v = (el && el.value || '').trim();
  if (v === SYSTEM_PIN) { window._systemUnlocked = true; render(); }
...
function render() {
  ...
  _hideSyncRefreshPill();   // v6.539: a real render makes the current screen fresh — clear any stale-data pill
  if (!state.user) {
    loginScreen();
    return;
  }
  ...
  // v6.625 — gate the whole System module behind the PIN (one unlock per visit to the section).
  if (isSystemRoute(state.route) && !window._systemUnlocked) {
    systemPinScreen(main, state.route);
  } else {
    handler(main);
  }
...
function renderSidebar() {
  ...
  if (flatNav) {
    ...
    // v6.626 — Collapse-all / Expand-all toolbar for the grouped menu. Sets every section's collapsed
    // state at once (the active section still opens so you never lose your place) + the footer "More".
    const _allSections = sections.filter(s => accessibleIn(s).length);
    const _ceBar = el('div', { className: 'nav-collapse-all' });
    _ceBar.style.cssText = 'display:flex;gap:6px;align-items:center;justify-content:flex-end;padding:0 14px 6px';
    const _mkCE = (icon, title, fn) => {
      const b = el('button', { title, className: 'nav-ce-btn' }, icon);
      b.style.cssText = 'background:transparent;border:1px solid var(--border);border-radius:6px;color:var(--text-mute);cursor:pointer;font-size:13px;line-height:1;padding:3px 8px';
      b.onclick = fn; return b;
    };
    _ceBar.append(_mkCE('⊟', t('Collapse all sections', 'طيّ كل الأقسام'), () => {
      try { localStorage.setItem('bs-nav-collapsed', JSON.stringify(_allSections)); localStorage.setItem('bs-foot-collapsed', '1'); } catch (_) {}
      render();
    }));
    _ceBar.append(_mkCE('⊞', t('Expand all sections', 'توسيع كل الأقسام'), () => {
      try { localStorage.setItem('bs-nav-collapsed', '[]'); localStorage.setItem('bs-foot-collapsed', '0'); } catch (_) {}
      render();
    }));
    nav.append(_ceBar);
...
const FR_STRINGS = {
  ...
  // ── v6.606 French batch 1 — parent-facing report + high-traffic UI ──
  "Attendance details report": "Rapport détaillé de présence",
  "Attendance Details Check": "Vérification détaillée de présence",
  "Details report": "Rapport détaillé",
  "Generated": "Généré le",
  "month(s)": "mois",
  "students": "élèves",
  "rows": "lignes",
  "classes attended in total": "cours suivis au total",
  "present total": "total des présences",
  "Student · Sport · Coach": "Élève · Sport · Entraîneur",
  "Total Y": "Total présents",
  "each cell shows the dates the student attended that month": "chaque case indique les dates de présence de l’élève ce mois-là",
  "each cell = the dates present that month": "chaque case = les dates de présence ce mois-là",
  "Membership = start → expiry": "Abonnement = début → expiration",
  "membership start → expiry": "début de l’abonnement → expiration",
  "Membership": "Abonnement",
  "multi-coach": "plusieurs entraîneurs",
  "No attendance recorded yet": "Aucune présence enregistrée pour le moment",
  "No students to export": "Aucun élève à exporter",
  "Detailed report over the selected months — the attended dates per student, sport and coach": "Rapport détaillé sur les mois sélectionnés — les dates de présence par élève, sport et entraîneur",
  "Cloud Storage Usage": "Utilisation du stockage cloud",
  "Storage": "Stockage",
  // Dashboard
  "Renewals": "Renouvellements",
  "membership renewed": "abonnement renouvelé",
  "memberships renewed": "abonnements renouvelés",
  "Memberships renewed in this period": "Abonnements renouvelés durant cette période",
  "Action items based on today's data — tap any to act": "Éléments à traiter selon les données d’aujourd’hui — appuyez pour agir",
  "Likely need renewal →": "À renouveler probablement →",
  "All clear — nothing needs attention right now": "Tout est en ordre — rien à traiter pour l’instant",
  "No expired memberships, none expiring soon, stock levels healthy": "Aucun abonnement expiré, aucun n’expire bientôt, stocks suffisants",
  "Members with expiry in the next 7 days — chase those renewals": "Membres expirant dans les 7 prochains jours — relancez ces renouvellements",
  "active members enrolled": "membres actifs inscrits",
  "accrual": "engagement",
  "cash paid": "payé en espèces",
  "payroll earned": "salaires acquis",
  "Show figures for this period": "Afficher les chiffres de cette période",
  "See how much data is stored in the cloud, by collection": "Voir la quantité de données stockées dans le cloud, par collection",
  "See how much data is stored in the cloud, broken down by collection (members, invoices, attendance, audit log…)": "Voir la quantité de données stockées dans le cloud, par collection (membres, factures, présences, journal d’audit…)",
  // Members / common
  "Search name, phone, QID, email...": "Rechercher nom, téléphone, QID, e-mail...",
  "Search…": "Rechercher…",
  "No matches": "Aucun résultat",
  "Filter by payment balance": "Filtrer par solde de paiement",
  "Filter by membership expiry": "Filtrer par expiration d’abonnement",
  "Switch between list and grid view": "Basculer entre vue liste et grille",
  "Switch between grid and list view": "Basculer entre vue grille et liste",
  "Show / hide table columns": "Afficher / masquer les colonnes",
  "Scan all members for duplicate phone numbers": "Rechercher les numéros en double chez tous les membres",
  "Switch sport / coach": "Changer de sport / entraîneur",
  "Delete permanently — cannot be undone": "Supprimer définitivement — irréversible",
  // Payments / collect
  "Charged": "Facturé",
  "Reconciles": "Concorde",
  "Charged = Paid + Due": "Facturé = Payé + Dû",
  "Collect a payment": "Encaisser un paiement",
  "applies to": "s’applique à",
  "Split across methods (e.g. part cash + part card)": "Répartir entre les modes (ex. partie espèces + partie carte)",
  "Split across methods (cash + card + fawran + transfer)": "Répartir entre les modes (espèces + carte + fawran + virement)",
  "Split the paid amount across methods (part cash + part card)": "Répartir le montant payé entre les modes (partie espèces + partie carte)",
  "Split this payout across methods (e.g. part cash + part transfer)": "Répartir ce versement entre les modes (ex. partie espèces + partie virement)",
  "Split total": "Total réparti",
  "replaces the paid breakdown for this invoice": "remplace la répartition payée de cette facture",
  "Enter an amount for at least one method": "Saisissez un montant pour au moins un mode",
  "No sport to collect for": "Aucun sport à encaisser",
  "Saved on this device but not yet in the cloud": "Enregistré sur cet appareil mais pas encore dans le cloud",
  "Enter an amount": "Saisissez un montant",
  "Paid now — amount per method": "Payé maintenant — montant par mode",
  "Installments": "Versements",
  "Edit prices": "Modifier les prix",
  "Record payment": "Enregistrer le paiement",
  "Still saving to the cloud — the connection is slow. Your change is safe on this device and keeps syncing automatically; you can keep working.": "Enregistrement dans le cloud toujours en cours — la connexion est lente. Votre modification est en sécurité sur cet appareil et se synchronise automatiquement ; vous pouvez continuer.",
  "Confirming it reached the server": "Vérification de la réception par le serveur",
  // Classes screen (from the screenshot)
  "Every scheduled class with its coach, sport, time and student list — print or export a register per class. Rosters are live from enrollments.": "Chaque cours planifié avec son entraîneur, sport, horaire et liste d’élèves — imprimez ou exportez une feuille par cours. Les listes proviennent en direct des inscriptions.",
  "from the weekly schedule": "d’après l’emploi du temps hebdomadaire",
  "across all shown classes": "sur tous les cours affichés",
  "planned, no students": "planifié, aucun élève",
  "Assign students": "Assigner des élèves",
};
function getLang() {
...
async function init() {
  ...
  if (window.Storage.isCloud()) {
    ...
    const _renderKeepScroll = () => {
      ...
      if (NO_AUTO_REPAINT_ROUTES.has(state.route)) { _remoteRenderPending = false; _showSyncRefreshPill(); return; }   // v6.539: don't rebuild the grid; offer a one-tap refresh so stale badges (e.g. UNPAID) can't mislead
      let y = 0;
    ...
    if (!window.ensureAuditLog) {
      ...
      window.ensureAuditLog = function (force) {
        // v6.580 — audit log OFF by default: don't fetch the (potentially huge) collection unless enabled.
        if (!(state.settings && state.settings.auditEnabled)) { if (!Array.isArray(state.auditLog)) state.auditLog = []; return Promise.resolve(state.auditLog); }
