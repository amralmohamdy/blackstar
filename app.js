...
function dedupeInvoicePayments(inv) {
  ...
  for (const p of inv.payments) {
    const base = p && p.pid ? String(p.pid).replace(/#\d+$/, '') : null;
    if (base) { if (seen.has(base)) { removed++; continue; } seen.add(base); }
    out.push(p);
  }
...
function isCurrentMember(m) {
  ...
  // v6.629 — match memberCounts().current EXACTLY (Active + Completed + Frozen) with a whitelist, not a
  // blacklist. The old `!== Expired && !== Withdrawn` let a TRANSFERRED-out member (status 'Transferred',
  // no enrollments) count as current, so the Dashboard sport donut + old coach's headcount read higher
  // than the Active KPI. A whitelist also auto-excludes any future status.
  const s = memberStatus(m);
  return s === 'Active' || s === 'Completed' || s === 'Frozen';
}
