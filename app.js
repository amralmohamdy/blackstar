...
async function withCloudConfirm(opts) {
  ...
  // The QC harness fires setTimeout SYNCHRONOUSLY, which would make the stall timer always beat the
  // (microtask-resolved) real save — so skip the artificial timer there and keep the original await.
  // A real browser never carries this UA, so production always gets the stall protection.
  const _syncTimers = (typeof navigator !== 'undefined' && navigator.userAgent === 'qc');
  let _slowT = null;
  const res = _syncTimers ? await saveConfirmed() : await Promise.race([
    ...
    new Promise(resolve => { _slowT = setTimeout(() => resolve({ ok: false, slow: true }), SLOW_SAVE_MS); }),
  ]);
