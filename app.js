...
function computeMonthlyPay(coachId, monthKey, uptoDate) {
  ...
  if (basis === 'attendance') {
    ...
    for (const inv of state.invoices) {
      // A DELETED / voided invoice earns the coach NOTHING (v6.449). This path only
      // skipped archived MEMBERS, not deleted INVOICES — so a member who had an invoice
      // deleted and re-generated (the common "fix the invoice" flow) kept BOTH the dead
      // and the live invoice in payroll, double-paying the coach (reported: Aziz / Kayid
      // Alshammari — deleted INV946349 + live INV946350 both counted → 2×960).
      if (inv.deleted) continue;
      ...
      for (const li of lineItems) {
        ...
        // "BY PAYMENT (full fee in payment month)" pays the coach the WHOLE line fee in the
        // month the membership was billed — attendance is IRRELEVANT (that is the ATTENDANCE
        // basis, handled above). The ONLY things that earn nothing are lines the eligibility
        // helper marks EXCLUDED: Summer Camp (already skipped) and an EXPIRED member with ZERO
        // attendance (never showed up — the v6.445 club rule). A FROZEN / partly-attended member
        // still counts at FULL fee here because they PAID this month. (v6.449 — was elig.base, the
        // attendance-prorated amount gated on elig.eligible, so a member billed 960 who had
        // attended 3/12 paid the coach 60%×240=144 instead of the full 60%×960=576.)
        const elig = lineCommissionEligibility(memForLine, inv, li, uptoDate);
        if (!elig.excluded) commissionBase += (parseFloat(li.price) || 0);
      }
    }
  }
